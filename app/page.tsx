...
export default async function Home({ searchParams }: { searchParams: { page: string, genre: string } }) {
  ...
  const getMoviesData = getMovies(searchParams.page, searchParams.genre)
  let movies
  ...
  return (
    <main className="">
      <div className="max-w-7xl mx-auto p-4">
        ...
        <ul className="grid grid-cols-5 gap-x-7 gap-y-8">{movies.results.map(movie => <MovieCard movie={movie} key={movie.id} favouriteMovies={favouriteMovies} />)}</ul>
