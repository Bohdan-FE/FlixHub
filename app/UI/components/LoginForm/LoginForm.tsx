    ...
    return (
        <form className='flex flex-col px-6 py-14 max-w-md mx-auto bg-neutral-800' action={dispatch}>
            ...
            {formState.message && <div className={clsx('border-solid border-neutral-300 border-2 p-3 mb-2 text-neutral-300 rounded-xl', {
                'border-red-300 p-3 mb-3 text-red-300': formState.message !== 'loged in'
            })}>{formState.message && <p className="text-center">{formState.message}</p>}</div>}
            <input className={styles.input} autoComplete='on' type="text" id='email' name="email" defaultValue={formState.fieldValues?.email} aria-describedby="email-error" placeholder="Email address" />
            ...
            <div className="relative">
                <input className={styles.input} type={isVisiblePassword ? 'text' : 'password'} autoComplete='off' id='password' name="password" defaultValue={formState.fieldValues?.password} aria-describedby="password-error" placeholder="Password" />
